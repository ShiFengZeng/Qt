...
void Widget::chooseSource(void)
{
    ...
    if (!fileName.isEmpty())
    {
        ...
        CalcHist(&srcImg, &hisImg, mtx, ui->toolButton_3);
    }
}


void Widget::CalcHist(QImage *Img, QImage *drawImg, unsigned int* hisMtx, QToolButton *button)
{
    int i, j;
    int tmpPix;
    unsigned int max = 0;
...
void Widget::histEq()
{
    ...
    CalcHist(&rsltImg, &hisEqImg, mtxEq, ui->toolButton_4);
}
    ...
    for(i = 0; i < hisSize; i++){
        if(hisMtx[i] != 0)
        {
            if(firstVal){
                ...
                CDFMtx[i] = hisMtx[i]; // initial value
            }
            firstVal = true;
        }
        else if (firstVal){
